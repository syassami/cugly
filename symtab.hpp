...
class SymName 
{
  const char* m_spelling; // "name" of the symbol
  ...
  SymName(const char* const x);
  ~SymName();
  virtual void accept(Visitor *v);
  virtual SymName *clone() const;
  void swap(SymName &);

  //fix me: add scope pointer here so that we don't need scope atrribute?
...
class Symbol
{
  //fix me: the interface to symbol should be more strictly enforced
  private:
  ...
  //fix me: this should be hidden by an interface
  ...
  int get_size() { 
    ...
    case bt_intarray:	return 0; // WRITEME
    case bt_procedure:	return 0;
    case bt_auto: return 12;
    case bt_undef:  return 12;
    default:		assert(0);
    }
  }
  ...
  int get_offset() { assert(m_offset!=-1); return m_offset; }
...
class SymTab
{
  ...
  // Returns the scope level the symbol is stored in.
  int get_lexical_level(Symbol *s);
  ...
  //returns the local size of the targetscope (in bytes)
  //in terms of the total amount of space that would be 
  //required to store all the local variables in that scope
